...
public:
	...
	//bool LoadPremiumServiceUser(const char* strAccountID, BYTE& bPremiumType, short& sPremiumTime);

	//BOOL LoadSavedMagic(char* charid, uint32_t* nSkill, uint32_t* nTime);

	//BOOL UpdateSavedMagic(char* charid, uint32_t* skill, uint32_t* time);
	...
	int NationSelectFix( char *id, char *pw );
