	...
	while(TRUE) {
		if( pMain->m_LoggerRecvQueue.GetFrontMode() != R ) {
			...
			case DB_COUPON_EVENT:
				...
				break;
			case WIZ_LETTER:
			//	pMain->CheckNewLetter( recv_buff+index );
				break;
			}
...
BOOL CAujardDlg::OnInitDialog()
{
	...
	wsprintf(strLogFile, "Log/AujardLog-%d-%d-%d.txt", time.GetYear(), time.GetMonth(), time.GetDay());
	...
	// Aujard tarafındaki düzeltilmiş hali:
	// 1. Ebenezer'in GÖNDERDİĞİNİ (SEND), Aujard ALIR (RECV)
	m_LoggerRecvQueue.InitailizeMMF(MAX_PKTSIZE, MAX_COUNT, "KNIGHT_SEND", FALSE);

	// 2. Aujard'ın CEVAPADIĞINI (SEND), Ebenezer ALIR (RECV)
	m_LoggerSendQueue.InitailizeMMF(MAX_PKTSIZE, MAX_COUNT, "KNIGHT_RECV", FALSE);
...
void CAujardDlg::SelectCharacter(char* pBuf)
{
	...
	sprintf(
		logstr,
		"SelectCharacter : account=%s char=%s uid=%d init=%d\r\n",
		accountid,
		userid,
		uid,
		bInit);
...
void CAujardDlg::SelectCharacter(char* pBuf)
{
	...
	accountLen = GetShort(pBuf, index);
	if (accountLen <= 0 || accountLen > MAX_ID_SIZE)
		goto fail_return;

	GetString(accountid, pBuf, accountLen, index);
	accountid[accountLen] = 0;

	charLen = GetShort(pBuf, index);
	if (charLen <= 0 || charLen > MAX_ID_SIZE)
		goto fail_return;

	GetString(userid, pBuf, charLen, index);
	userid[charLen] = 0;
	...
	sprintf(
		logstr,
		"SelectCharacter : account=%s char=%s uid=%d init=%d packetIndex=%d\r\n",
		accountid,
		userid,
		uid,
		bInit,
	...
	if (GetUserPtr(userid, old_uid))
	{
		send_index = 0;

		SetShort(send_buff, old_uid, send_index);

		SetShort(send_buff, accountLen, send_index);
		SetString(send_buff, accountid, accountLen, send_index);

		SetShort(send_buff, charLen, send_index);
		SetString(send_buff, userid, charLen, send_index);
	...
	if (!m_DBAgent.LoadUserData(accountid, userid, uid))
	{
		WriteLogFile("LoadUserData FAILED\r\n");
		goto fail_return;
	}
	...
	if (!m_DBAgent.LoadWarehouseData(accountid, uid))
	{
		WriteLogFile("LoadWarehouseData FAILED\r\n");
		goto fail_return;
	}
	...
	// ZONE
	SetByte(send_buff, pUser->m_bZone, send_index);

	// POSITION
	SetShort(send_buff, (short)(pUser->m_curx * 10), send_index);
	SetShort(send_buff, (short)(pUser->m_curz * 10), send_index);
	SetShort(send_buff, (short)(pUser->m_cury * 10), send_index);

	// WINNING NATION
	//SetByte(send_buff, m_byOldVictory, send_index);
	...
	if (count >= 50)
	{
		WriteLogFile("SelectCharacter Packet Drop!!!\r\n");
	}

	WriteLogFile("SelectCharacter SUCCESS\r\n");
...
fail_return:
	...
	WriteLogFile("SelectCharacter FAILED\r\n");
}
*/
...
void CAujardDlg::AccountLogIn(char* pBuf)
{
	...
	nation = m_DBAgent.NationSelectFix(accountid, password);
...
void CAujardDlg::DeleteChar(char *pBuf)
{
	...
	printf("*** DeleteChar == charid=%s, socno=%s ***\n", charid, socno);
...
void CAujardDlg::AllSaveRoutine()
{
	...
	for( int i=0; i<t_count; i++ ) {
		...
		if( m_DBAgent.UpdateUser( pUser->m_id, i, UPDATE_ALL_SAVE ) ) {
			printf("GameServer Dead!! - %s Saved\n", pUser->m_id );
			Sleep(100);
			count = 0;
			do {
			...
			printf("GameServer Dead!! - %s Not Saved\n", pUser->m_id );

		Sleep(100);
	}
	if( bsaved ) {
		msgstr.Format("All Data Saved:%d³â%d¿ù%dÀÏ%d½Ã%dºĞ", cur.GetYear(), cur.GetMonth(), cur.GetDay(), cur.GetHour(), cur.GetMinute());
...
void CAujardDlg::ConCurrentUserCount()
{
	...
	printf("*** ConCurrentUserCount : server=%d, zone=%d, usercount=%d ***\n", m_nServerNo, m_nZoneNo, t_count);
...
void CAujardDlg::CreateKnights(char *pBuf)
{
	...
	printf("CreateKnights - nid=%d, index=%d, result=%d \n", uid, knightindex, result);
...
void CAujardDlg::JoinKnights(char *pBuf)
{
	...
	printf("JoinKnights - nid=%d, name=%s, index=%d, result=%d \n", uid, pUser->m_id, knightindex, result);
...
void CAujardDlg::WithdrawKnights(char *pBuf)
{
	...
	printf("WithDrawKnights - nid=%d, index=%d, result=%d \n", uid, knightindex, result);
...
/*	if( remove_flag == 0 && command == KNIGHTS_REMOVE )	{		// ¾ø´Â À¯Àú Ãß¹æ½Ã¿¡´Â µğºñ¿¡¼­¸¸ Ã³¸®ÇÑ´Ù
		result = m_DBAgent.UpdateKnights( command, userid, knightindex, remove_flag );
		printf("ModifyKnights - command=%d, nid=%d, index=%d, result=%d \n", command, uid, knightindex, result);
		return;
	}	*/
...
void CAujardDlg::DestroyKnights(char *pBuf)
{
	...
	printf("DestoryKnights - nid=%d, index=%d, result=%d \n", uid, knightindex, result);
...
void CAujardDlg::WriteLogFile( char* pData )
{
	...
	if( m_iLogFileDay != nDay )	{
		...
		wsprintf(strLog, "Log/AujardLog-%d-%d-%d.txt", cur.GetYear(), cur.GetMonth(), cur.GetDay());
	...
	if( nLen >= 1024 )	{
		printf("### WriteLogFile Fail : length = %d ###\n", nLen);
		return;
	}
...
void CAujardDlg::BattleEventResult( char* pData )
{
	...
	if( nLen > 0 && nLen < MAX_ID_SIZE+1 )	{
		...
		printf("--> BattleEventResult : Àû±¹ÀÇ ´ëÀåÀ» Á×ÀÎ À¯ÀúÀÌ¸§Àº? %s, len=%d, nation=%d \n", strMaxUserName, nResult, nResult);
