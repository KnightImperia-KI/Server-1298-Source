...
void CNpc::FillNpcInfo(char* temp_send, int& index, BYTE flag)
{
	...
	if (m_bySpecialType == 5 && m_byChangeType == 0)
		...
		SetByte(temp_send, 1, index);

	printf("NPC INFO START nid=%d pid=%d sid=%d type=%d zone=%d mapPtr=%p\n",
		m_sNid,
		m_sPid,
		m_sSid,
		m_tNpcType,
	...
	printf("NPC POS x=%.2f z=%.2f y=%.2f dir=%.2f\n",
		m_fCurX,
		m_fCurZ,
		m_fCurY,
		m_fDir);

	// ✔ POSITION (FLOAT DEĞİL SHORT*10)
	SetShort(temp_send, (short)(m_fCurX * 10), index);
	SetShort(temp_send, (short)(m_fCurZ * 10), index);
	SetShort(temp_send, (short)(m_fCurY * 10), index);

	// dir FLOAT DEĞİL
	SetByte(temp_send, (BYTE)m_fDir, index);

	// HP CHECK
	SetByte(temp_send, (m_iHP > 0 ? 1 : 0), index);
...
void CNpc::SendNpcInfoAll(char* temp_send, int& index, int count)
{
	...
	// RESOURCE / PICTURE ID used by the 1298 client
	SetShort(temp_send, m_sPid, index);
	...
	// STATUS
	SetDWORD(temp_send, (DWORD)m_byGateOpen, index);

	// TYPE
	SetByte(temp_send, m_byObjectType, index);

	// UNKNOWNS + DIR
	SetShort(temp_send, 0, index);
	SetShort(temp_send, 0, index);
	SetByte(temp_send, (BYTE)m_fDir, index);
}
