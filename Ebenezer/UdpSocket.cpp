...
void CUdpSocket::RecvBattleEvent(char *pBuf)
{
	...
	if( nType == BATTLE_EVENT_OPEN )	{
		...
		if( m_pMain->m_byBattleOpen == NO_BATTLE )	{
			printf("#### UDP RecvBattleEvent Fail : battleopen = %d, type = %d\n", m_pMain->m_byBattleOpen, nType);
			return;
		}
		if( nResult == KARUS )	{
			//printf("--> UDP RecvBattleEvent : 카루스 땅으로 넘어갈 수 있어\n");
			m_pMain->m_byKarusOpenFlag = 1;		// 카루스 땅으로 넘어갈 수 있어
		}
		else if( nResult == ELMORAD )	{
			//printf("--> UDP  RecvBattleEvent : 엘모 땅으로 넘어갈 수 있어\n");
		...
		if( nResult == KARUS )	{
			//printf("-->  UDP RecvBattleEvent : 카루스가 승리하였습니다.\n");
		}
		else if( nResult == ELMORAD )	{
			//printf("-->  UDP RecvBattleEvent : 엘모라드가 승리하였습니다.\n");
		}
		...
		if( nLen > 0 && nLen < MAX_ID_SIZE+1 )	{
			...
			//printf("-->  UDP RecvBattleEvent : 적국의 대장을 죽인 유저이름은? %s, len=%d\n", strMaxUserName, nResult);
			if( nResult == 1 )	{
...
void CUdpSocket::ReceiveKnightsProcess( char* pBuf )
{
	...
	command = GetByte( pBuf, index );
	//printf("UDP - ReceiveKnightsProcess - command=%d\n", command);
...
void CUdpSocket::RecvCreateKnights( char* pBuf )
{
	...
	//printf("UDP - RecvCreateKnights - knname=%s, name=%s, index=%d\n", knightsname, chiefname, knightsindex);
}
...
void CUdpSocket::RecvJoinKnights( char* pBuf, BYTE command )
{
	...
	if( command == KNIGHTS_JOIN ) {
		...
		printf("UDP - RecvJoinKnights - 가입, name=%s, index=%d\n", charid, knightsindex);
	}
	else {		// 탈퇴..
		// 클랜정보에 추가
		m_pMain->m_KnightsManager.RemoveKnightsUser( knightsindex, charid );
		sprintf( finalstr, "#### %s님이 탈퇴하셨습니다. ####", charid );
		printf("UDP - RecvJoinKnights - 탈퇴, name=%s, index=%d\n", charid, knightsindex );
	}

	//printf("UDP - RecvJoinKnights - command=%d, name=%s, index=%d\n", command, charid, knightsindex);
...
void CUdpSocket::RecvDestroyKnights( char* pBuf )
{
	...
	pKnights = m_pMain->m_KnightsArray.GetData( knightsindex );
	if( !pKnights )		{
		printf("UDP - ### RecvDestoryKnights  Fail == index = %d ###\n", knightsindex);
		return;
	}
	...
	//printf("UDP - RecvDestoryKnights - index=%d\n", knightsindex);
}
...
void CUdpSocket::RecvBattleZoneCurrentUsers( char* pBuf )
{
	...
	// N--E--R--O
	//printf("UDP - RecvBattleZoneCurrentUsers - karus=%d, elmorad=%d\n", nKarusMan, nElmoradMan);
}
