		...
		if( pMain->m_LoggerRecvQueue.GetFrontMode() != R ) {
			...
			switch( command ) {
				...
				case DB_COUPON_EVENT:
					...
					break;
				case WIZ_LETTER:
					pUser->CheckNewLetter( pBuf+index );
					break;
			}
		}
		else
	...
	: CDialog(CEbenezerDlg::IDD, pParent)
{
	...
	for(int e=0; e<20; e++) {
		memset( m_epNotice[e], NULL, 128 );
		memset( m_upNotice[e], NULL, 128 );
	}
...
/*	if( AfxMessageBox("If you are restarting, please restart after all data is saved...do you want to continue?", MB_OKCANCEL) == IDCANCEL ) {
	...
	wsprintf(strLogFile, "Log/RegionLog-%d-%d-%d.txt", cur.GetYear(), cur.GetMonth(), cur.GetDay());
	...
	wsprintf(strLogFile, "Log/PacketLog-%d-%d-%d.txt", cur.GetYear(), cur.GetMonth(), cur.GetDay());
	m_LogFile.Open(strLogFile, CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone);
	m_LogFile.SeekToEnd();

	wsprintf(strLogFile, "Log/SpeedHackLog-%d-%d-%d.txt", cur.GetYear(), cur.GetMonth(), cur.GetDay());
	m_LogFile.Open(strLogFile, CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone);
	m_LogFile.SeekToEnd();

	wsprintf(strLogFile, "Log/LoginLog-%d-%d-%d.txt", cur.GetYear(), cur.GetMonth(), cur.GetDay());
	m_LogFile.Open(strLogFile, CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone);
	m_LogFile.SeekToEnd();

	wsprintf(strLogFile, "Log/ReportBugLog-%d-%d-%d.txt", cur.GetYear(), cur.GetMonth(), cur.GetDay());
	m_LogFile.Open(strLogFile, CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone);
	m_LogFile.SeekToEnd();

	wsprintf(strLogFile, "Log/AIServerLog-%d-%d-%d.txt", cur.GetYear(), cur.GetMonth(), cur.GetDay());
	m_LogFile.Open(strLogFile, CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone);
	m_LogFile.SeekToEnd();
	...
	wsprintf(strLogFile, "Log/EbenezerLog-%d-%d-%d.txt", cur.GetYear(), cur.GetMonth(), cur.GetDay());
	m_LogFile.Open(strLogFile, CFile::modeWrite | CFile::modeCreate | CFile::modeNoTruncate | CFile::shareDenyNone);
	m_LogFile.SeekToEnd();
	...
	LogFileWrite("before KnightsData");
	...
	LogFileWrite("before LoadAllKnightsUserData");
	...
	LogFileWrite("before LoadHomeTable");
	...
	LogFileWrite("before START POSITION");
	if (!LoadStartPositionTable())
		return FALSE;

	LogFileWrite("before LoadBattleTable");
	...
	if (LoadServerResourceTable() == FALSE) {
		AfxMessageBox("ServerSource Load Fail");
		AfxPostQuitMessage(0);
		return FALSE;
	}
...
//	LogFileGameServer("success\r\n");
	UserAcceptThread();

	//CTime cur = CTime::GetCurrentTime();
	CString starttime;
	starttime.Format("Game Server Start : M%d D%d H%d M%d\r\n", cur.GetMonth(), cur.GetDay(), cur.GetHour(), cur.GetMinute());
	LogFileWrite( (char*)(LPCTSTR)starttime );
	m_StatusList.AddString(starttime);


	CString strVersion;
	strVersion.Format(_T("%d"), __VERSION);
	SetDlgItemText(1007, strVersion); // 1007, görseldeki "Ver." kutusunun ID'si
	// Mevcut strVersion kodlarını silip direkt bunu yazabilirsiniz:
	SetDlgItemText(1009, _T("1"));

	int onlineUser = 0;

	for (int i = 0; i < m_Iocport.m_SocketArraySize; i++)
	{
		if (m_Iocport.m_SockArray[i] != NULL)
			onlineUser++;
	}

	CString strUserStatus;
	strUserStatus.Format(_T("%d, %d"), onlineUser, MAX_USER);

	SetDlgItemText(1015, strUserStatus);

	// Battle Start Time (1010)
	TCHAR szWeek[10], szStart[10], szEnd[10];
	CString strBattle;

	// 1. INI dosyasından değerleri oku
	GetPrivateProfileString(_T("BATTLE"), _T("WEEK"), _T("0"), szWeek, 10, _T(".\\GameServer.ini"));
	GetPrivateProfileString(_T("BATTLE"), _T("START_TIME"), _T("0"), szStart, 10, _T(".\\GameServer.ini"));
	GetPrivateProfileString(_T("BATTLE"), _T("END_TIME"), _T("0"), szEnd, 10, _T(".\\GameServer.ini"));

	// 2. Formatı oluştur: W=5-S=20E=0
	strBattle.Format(_T("t=%s - a=%s -v=%s"), szWeek, szStart, szEnd);

	// 3. 1010 ID'li kutuya yazdır
	SetDlgItemText(1010, strBattle);
	CString str;
	str.Format(_T(" t:0, s:0, r:0"));

	SetDlgItemText(1011, str);

	CString sstr;
	sstr.Format(_T("%d"), 0);

	SetDlgItemText(1012, sstr);

	CString hstr;
	hstr.Format(_T("%d"), 0);

	SetDlgItemText(1013, hstr);

	SetDlgItemText(1014, _T("0,0"));

	return TRUE;  // return TRUE  unless you set the focus to a control
}
void CEbenezerDlg::RefreshStatus()
{
	
	CString strVersion;
	strVersion.Format(_T("%d"), __VERSION);
	SetDlgItemText(1007, strVersion);
	...
	GetPrivateProfileString(_T("BATTLE"), _T("WEEK"), _T("0"),
		szWeek, 10, _T(".\\GameServer.ini"));

	GetPrivateProfileString(_T("BATTLE"), _T("START_TIME"), _T("0"),
		szStart, 10, _T(".\\GameServer.ini"));

	GetPrivateProfileString(_T("BATTLE"), _T("END_TIME"), _T("0"),
		szEnd, 10, _T(".\\GameServer.ini"));
	...
	// Siege / Debug fields
	SetDlgItemText(1011, _T("t:0, s:0, r:0"));
	SetDlgItemText(1012, _T("0"));
	SetDlgItemText(1013, _T("0"));
	SetDlgItemText(1014, _T("0,0"));
}

void CEbenezerDlg::OnBnClicked1008() // 1008 ID'li Buton
{
	RefreshStatus();
}
...
void CEbenezerDlg::OnTimer(UINT nIDEvent) 
{
	...
	case SEND_TIME:
		...
		if ( !retval ) {
			...
			printf("Send PostQueued Error : %d\n", errValue);
		}
		break;
	case ALIVE_TIME:
		CheckAliveUser();
...
BOOL CEbenezerDlg::AISocketConnect(int zone, int flag)
{
	...
	if(flag == 1)	SetByte(pBuf, 1, send_index);			// ÀçÁ¢¼Ó
	else			SetByte(pBuf, 0, send_index);			// Ã³À½ Á¢¼Ó..
	...
	printf("**** AISocket Connect Success!! ,, zone = %d ****\n", zone);
	return TRUE;
}
// ~sungyong 2002.05.22
...
{
	if (!pBuf || len <= 0 || len > 8192)
		return;

	if (zoneindex < 0 || zoneindex >= (int)m_ZoneArray.size())
		return;

	C3DMap* pMap = m_ZoneArray[zoneindex];
	if (!pMap || !pMap->m_ppRegion)
		return;
	...
	auto& userMap = pMap->m_ppRegion[x][z].m_RegionUserArray.m_UserTypeMap;
	...
	for (auto it = userMap.begin(); it != userMap.end(); ++it)
	{
		...
		// 🔥 DIRECT SOCKET ARRAY ACCESS (DOĞRU YOL)
	...
	for(int i=0; i<MAX_AI_SOCKET; i++) {
		...
		if( i == m_sSendSocket )	{
			...
			if(send_size == 0)	continue;
				...
				//printf(" <--- Send_AIServer : length = %d, socket = %d \n", send_size, old_send_socket);
				return;
			}
		}
		else	{
...
BOOL CEbenezerDlg::LoadStartPositionTable()
{
	...
	while (!StartPositionSet.IsEOF())
	{
		...
		{
			printf("Could not add zone %d to the starting position array\n", pData->ZoneID);
			delete pData;
		}
...
BOOL CEbenezerDlg::LoadItemTable()
{
	...
	while( !ItemTableSet.IsEOF() )
	{
		...
		if( !m_ItemtableArray.PutData(pTableItem->m_iNum, pTableItem) ) {
			printf("ItemTable PutData Fail - %d\n", pTableItem->m_iNum );
			delete pTableItem;
			pTableItem = NULL;
		}
...
BOOL CEbenezerDlg::LoadMagicTable()
{
	...
	while( !MagicTableSet.IsEOF() )
	{
		...
		if( !m_MagictableArray.PutData(pTableMagic->iNum, pTableMagic) ) {
			printf("MagicTable PutData Fail - %d\n", pTableMagic->iNum );
			delete pTableMagic;
			pTableMagic = NULL;
		}
...
BOOL CEbenezerDlg::LoadMagicType1()
{
	...
	while( !MagicType1Set.IsEOF() )
	{
		...
		if( !m_Magictype1Array.PutData(pType1Magic->iNum, pType1Magic) ) {
			printf("MagicType1 PutData Fail - %d\n", pType1Magic->iNum );
			delete pType1Magic;
			pType1Magic = NULL;
		}
...
BOOL CEbenezerDlg::LoadMagicType2()
{
	...
	while( !MagicType2Set.IsEOF() )
	{
		...
		if( !m_Magictype2Array.PutData(pType2Magic->iNum, pType2Magic) ) {
			printf("MagicType2 PutData Fail - %d\n", pType2Magic->iNum );
			delete pType2Magic;
			pType2Magic = NULL;
		}
...
BOOL CEbenezerDlg::LoadMagicType3()
{
	...
	while( !MagicType3Set.IsEOF() )
	{
		...
		if( !m_Magictype3Array.PutData(pType3Magic->iNum, pType3Magic) ) {
			printf("MagicType3 PutData Fail - %d\n", pType3Magic->iNum );
			delete pType3Magic;
			pType3Magic = NULL;
		}	
...
BOOL CEbenezerDlg::LoadMagicType4()
{
	...
	while( !MagicType4Set.IsEOF() )
	{
		...
		{
			printf("MagicType4 PutData Fail - %d\n", pType4Magic->iNum );
			delete pType4Magic;
			pType4Magic = NULL;
		}	
...
BOOL CEbenezerDlg::LoadMagicType5()
{
	...
	while( !MagicType5Set.IsEOF() )
	{
		...
		if( !m_Magictype5Array.PutData(pType5Magic->iNum, pType5Magic) ) {
			printf("MagicType5 PutData Fail - %d\n", pType5Magic->iNum );
			delete pType5Magic;
			pType5Magic = NULL;
		}	
...
BOOL CEbenezerDlg::LoadMagicType8()
{
	...
	while( !MagicType8Set.IsEOF() )
	{
		...
		if( !m_Magictype8Array.PutData(pType8Magic->iNum, pType8Magic) ) {
			printf("MagicType8 PutData Fail - %d\n", pType8Magic->iNum );
			delete pType8Magic;
			pType8Magic = NULL;
		}
...
BOOL CEbenezerDlg::LoadCoefficientTable()
{
	...
	while( !CoefficientSet.IsEOF() )
	{
		...
		if( !m_CoefficientArray.PutData(p_TableCoefficient->sClassNum, p_TableCoefficient) ) {
			printf("Coefficient PutData Fail - %d\n", p_TableCoefficient->sClassNum );
...
void CEbenezerDlg::UserInOutForMe(CUser *pSendUser)
{
	...
	if( prev_index >=  49152) {
		printf("#### UserInOutForMe - buffer overflow = %d ####\n", prev_index);
		return;
	}
	SetString( send_buff, buff, buff_index, send_index );
	memset( buff, NULL, 16384 );
...
void CEbenezerDlg::RegionUserInOutForMe(CUser* pSendUser)
{
	...
	char uid_buff[2048];
	char send_buff[16384];

	ZeroMemory(uid_buff, sizeof(uid_buff));
	...
	send_index = 3; // opcode + count reserved

	auto AddRegion = [&](int rx, int rz)
		{
			if (rx < 0 || rz < 0 ||
			...
			buff_index = GetRegionUserList(pMap, rx, rz, uid_buff, t_count);

			if (buff_index <= 0)
				return;

			SetString(send_buff, uid_buff, buff_index, send_index);
		};

	AddRegion(pSendUser->m_RegionX, pSendUser->m_RegionZ);
	AddRegion(pSendUser->m_RegionX - 1, pSendUser->m_RegionZ - 1);
	AddRegion(pSendUser->m_RegionX, pSendUser->m_RegionZ - 1);
	AddRegion(pSendUser->m_RegionX + 1, pSendUser->m_RegionZ - 1);

	AddRegion(pSendUser->m_RegionX - 1, pSendUser->m_RegionZ);
	AddRegion(pSendUser->m_RegionX + 1, pSendUser->m_RegionZ);
	...
	SetShort(send_buff, 1, temp_index);
	SetShort(send_buff, t_count, temp_index);
...
int CEbenezerDlg::GetRegionUserIn(C3DMap* pMap, int region_x, int region_z, char* buff, int& t_count)
{
	...
	if (!pMap) return 0;
	if (region_x < 0 || region_z < 0 || region_x >= pMap->GetXRegionMax() || region_z >= pMap->GetZRegionMax()) return 0;

	EnterCriticalSection(&g_region_critical);

	map < int, int* >::iterator Iter1 = pMap->m_ppRegion[region_x][region_z].m_RegionUserArray.m_UserTypeMap.begin();
	map < int, int* >::iterator Iter2 = pMap->m_ppRegion[region_x][region_z].m_RegionUserArray.m_UserTypeMap.end();
	...
	for (; Iter1 != Iter2; Iter1++) {
		...
		// --- PAKET BAŞLANGICI ---
		SetByte(buff, 0x00, buff_index); // Type (User)
		...
		// --- CLAN BİLGİLERİ (Hizalama Önemli) ---
		...
		if (pKnights) {
			...
			SetShort(buff, 0, buff_index); // Knights Symbol/Mark
		}
		else {
			SetByte(buff, 0, buff_index); // Rank 0
			SetByte(buff, 0, buff_index); // Name Len 0
		...
		// Görsel Durumlar (HP Bar, Poison efekti vb.)
		...
		// --- ITEMLAR (Kıyafetler ve Silahlar) ---
		// 1298 Client bu 8 slotun ID ve Durability bilgisini bekler
		int items[] = { BREAST, LEG, HEAD, GLOVE, FOOT, SHOULDER, RIGHTHAND, LEFTHAND };
		for (int k = 0; k < 8; k++) {
			SetDWORD(buff, pUser->m_pUserData->m_sItemArray[items[k]].nNum, buff_index);
			SetShort(buff, pUser->m_pUserData->m_sItemArray[items[k]].sDuration, buff_index);
		}
...
void CEbenezerDlg::RegionNpcInfoForMe(CUser* pSendUser, int nType)
{
	...
	if (pSendUser->m_iZoneIndex < 0 || pSendUser->m_iZoneIndex >= (int)m_ZoneArray.size())
		return;

	C3DMap* pMap = (C3DMap*)m_ZoneArray[pSendUser->m_iZoneIndex];
	if (!pMap)
		return;
	...
	SetByte(send_buff, (nType == 1 ? WIZ_TEST_PACKET : WIZ_NPC_REGION), send_index);

	int count_index = send_index;
	SetShort(send_buff, 0, send_index); // total NPC placeholder

	auto SendRegion = [&](int rx, int rz)
		{
			if (rx < 0 || rz < 0 ||
				rx >= pMap->GetXRegionMax() ||
				rz >= pMap->GetZRegionMax())
			...
			char npc_buff[4096];
			memset(npc_buff, 0, sizeof(npc_buff));

			int local_count = 0;
			int size = GetRegionNpcList(pMap, rx, rz, npc_buff, local_count, nType);

			if (size > 0 && local_count > 0)
			{
				SetString(send_buff, npc_buff, size, send_index);
				total_npc += local_count;
			}
		};

	int rx = pSendUser->m_RegionX;
	int rz = pSendUser->m_RegionZ;
	...
	for (int dx = -1; dx <= 1; dx++)
	{
		...
		{
			SendRegion(rx + dx, rz + dz);
		}
	}

	// total NPC write-back
	*(short*)(send_buff + count_index) = total_npc;
...
int CEbenezerDlg::GetRegionNpcList(C3DMap* pMap, int region_x, int region_z, char* nid_buff, int& t_count, int nType)
{
	...
	auto it = pMap->m_ppRegion[region_x][region_z].m_RegionNpcArray.m_UserTypeMap.begin();
	auto end = pMap->m_ppRegion[region_x][region_z].m_RegionNpcArray.m_UserTypeMap.end();
	...
	for (; it != end; it++)
	{
		...
		// =========================
		// FIX: CLIENT STABLE FORMAT
		// =========================

		SetShort(nid_buff, pNpc->m_sNid, buff_index);
		SetShort(nid_buff, pNpc->m_sPid, buff_index);

		SetByte(nid_buff, pNpc->m_tNpcType, buff_index);
		SetByte(nid_buff, pNpc->m_byGroup, buff_index);

		SetShort(nid_buff, (short)(pNpc->m_fCurX * 10), buff_index);
		SetShort(nid_buff, (short)(pNpc->m_fCurZ * 10), buff_index);
...
/*
int CEbenezerDlg::GetRegionNpcList(C3DMap* pMap, int region_x, int region_z, char* buff, int& t_count, int nType)
{
	if (!m_bPointCheckFlag || !pMap)
		return 0;
	...
	auto& region = pMap->m_ppRegion[region_x][region_z].m_RegionNpcArray.m_UserTypeMap;
	...
	for (auto it = region.begin(); it != region.end(); ++it)
	{
		...
		CNpc* pNpc = (CNpc*)m_arNpcArray.GetData(nid);
		if (!pNpc)
			continue;

		// 💥 IMPORTANT: DEAD NPC FILTER
		if (pNpc->m_iHP <= 0)
			continue;
...
void CEbenezerDlg::NpcInOutForMe(CUser* pSendUser)
{
	...
	C3DMap* pMap = m_ZoneArray[pSendUser->m_iZoneIndex];
	if (!pMap)
		return;
	...
	SetByte(send_buff, WIZ_REQ_NPCIN, send_index);
	int count_index = send_index;
	SetShort(send_buff, 0, send_index); // placeholder
	...
	// write real count
	SetShort(send_buff, t_count, count_index);

	pSendUser->SendCompressingPacket(send_buff, send_index);
}

int CEbenezerDlg::GetRegionNpcIn(
	C3DMap* pMap,
	int region_x,
	int region_z,
	char* buff,
	int& t_count)
{
	...
	// ✅ DOĞRU CONTAINER
	auto& npcMap = pRegion->m_RegionNpcArray.m_UserTypeMap;
	...
	for (auto it = npcMap.begin(); it != npcMap.end(); ++it)
	{
		if (!it->second)
			continue;
		...
		if (buff_index + 128 >= 4096)
			break;

		SetShort(buff, pNpc->m_sNid, buff_index);
		SetShort(buff, pNpc->m_sSid, buff_index);
		...
		char szName[32] = { 0 };
		strcpy_s(szName, pNpc->m_strName);

		BYTE len = (BYTE)strlen(szName);
		SetByte(buff, len, buff_index);

		memcpy(buff + buff_index, szName, len);
		buff_index += len;

		SetByte(buff, pNpc->m_byGroup, buff_index);
		SetByte(buff, pNpc->m_byLevel, buff_index);

		SetShort(buff, (short)(pNpc->m_fCurX * 10), buff_index);
		SetShort(buff, (short)(pNpc->m_fCurZ * 10), buff_index);
		SetShort(buff, (short)(pNpc->m_fCurY * 10), buff_index);

		SetByte(buff, pNpc->m_byGateOpen, buff_index);
...
void CEbenezerDlg::RegionNpcInfoForMe(CUser* pSendUser, int nType)
{
	...
	char send_buff[8192];
	char region_buff[4096];

	memset(send_buff, 0, sizeof(send_buff));
	memset(region_buff, 0, sizeof(region_buff));

	int send_index = 0;

	// HEADER
	SetByte(send_buff, WIZ_NPC_REGION, send_index);

	// NPC COUNT PLACEHOLDER
	int count_index = send_index;
	...
	for (int x = rx - 1; x <= rx + 1; x++)
	{
		for (int z = rz - 1; z <= rz + 1; z++)
		{
			...
			int region_size = GetRegionNpcList(
				pMap,
				x,
				z,
				region_buff,
			...
			if (send_index + region_size >= sizeof(send_buff))
				continue;

			memcpy(send_buff + send_index, region_buff, region_size);
			send_index += region_size;
		}
	}

	// NPC COUNT FIX
	int temp = count_index;
	SetShort(send_buff, npc_count, temp);

	printf("SEND NPC REGION user=%s totalNpc=%d packetSize=%d region=(%d,%d)\n",
...
void CEbenezerDlg::RegionNpcInfoForMe(CUser* pSendUser, int nType)
{
	...
	// =========================
	if (nType == 1)
		SetByte(send_buff, WIZ_TEST_PACKET, send_index);
	else
		SetByte(send_buff, WIZ_NPC_REGION, send_index);
	...
	// REGION CHECK (9 grid)
	// =========================
	int base_rx = pSendUser->m_RegionX;
	int base_rz = pSendUser->m_RegionZ;
	...
	for (int i = 0; i < 9; i++)
	{
		int rx = base_rx + dirs[i][0];
		int rz = base_rz + dirs[i][1];

		buff_index = GetRegionNpcList(
			pMap,
			rx,
			rz,
			nid_buff,
		...
		if (buff_index > 0)
		{
			SetString(send_buff, nid_buff, buff_index, send_index);
			memset(nid_buff, 0, sizeof(nid_buff));
		}
	}
	...
	// FINAL COUNT FIX
	// =========================
	SetShort(send_buff, npc_count, count_index);
	...
	if (npc_count <= 0)
	{
		printf("NPC REGION EMPTY zoneIndex=%d rx=%d rz=%d\n",
			pSendUser->m_iZoneIndex,
			pSendUser->m_RegionX,
			pSendUser->m_RegionZ);
	}
	else if (npc_count > 500)
	{
		printf("WARNING NPC COUNT HIGH: %d\n", npc_count);
	}
}
/*
int CEbenezerDlg::GetRegionNpcList(
	C3DMap* pMap,
	int region_x,
	int region_z,
	char* buff,
	...
	int nType)
{
	...
	auto& mp = pRegion->m_RegionNpcArray.m_UserTypeMap;
	...
	for (auto itr = mp.begin(); itr != mp.end(); ++itr)
	{
		...
		// =====================================================
		// PACKET (CLIENT FORMAT EXACT MATCH)
		// =====================================================

		SetShort(buff, pNpc->m_sNid, index);
		SetShort(buff, pNpc->m_sSid, index);

		SetByte(buff, pNpc->m_tNpcType, index);
		SetDWORD(buff, pNpc->m_iSellingGroup, index);

		SetShort(buff, pNpc->m_sSize, index);

		SetDWORD(buff, pNpc->m_iWeapon_1, index);
		SetDWORD(buff, pNpc->m_iWeapon_2, index);

		// NAME
		int len = strlen(pNpc->m_strName);
		if (len < 0) len = 0;
		if (len > 30) len = 30;

		SetByte(buff, len, index);
		memcpy(buff + index, pNpc->m_strName, len);
		index += len;

		SetByte(buff, pNpc->m_byGroup, index);
		SetByte(buff, pNpc->m_byLevel, index);

		// POSITION (IMPORTANT FIX - float*10 -> short)
		SetShort(buff, (short)(pNpc->m_fCurX * 10), index);
		SetShort(buff, (short)(pNpc->m_fCurZ * 10), index);
		SetShort(buff, (short)(pNpc->m_fCurY * 10), index);

		SetByte(buff, pNpc->m_byGateOpen, index);
		SetByte(buff, pNpc->m_byObjectType, index);

		// EXTRA (CLIENT SKIP FIELDS)
		SetShort(buff, 0, index);
		SetShort(buff, 0, index);
		SetByte(buff, 0, index);
...
BOOL CEbenezerDlg::LoadEventNoticeData()
{
	CString ProgPath = GetProgPath();
	CString NoticePath = ProgPath + "eventnotice.txt";
	CString buff;
	CStdioFile txt_file;
	int count = 0;

	if (!txt_file.Open(NoticePath, CFile::modeRead)) {
		AfxMessageBox("cannot open eventnotice.txt!!");
		return FALSE;
	}

	while( txt_file.ReadString(buff) ) {
		if( count > 19 )
			AfxMessageBox("Event Notice Count Overflow!!");
		strcpy( m_epNotice[count], (char*)(LPCTSTR)buff );
		count++;
	}
...
BOOL CEbenezerDlg::LoadNoticeUpData()
{
	CString ProgPath = GetProgPath();
	CString NoticePath = ProgPath + "Notice_up.txt";
	CString buff;
	CStdioFile txt_file;
	int count = 0;

	if (!txt_file.Open(NoticePath, CFile::modeRead)) {
		AfxMessageBox("cannot open Notice_up.txt!!");
		return FALSE;
	}

	while( txt_file.ReadString(buff) ) {
		if( count > 19 )
			AfxMessageBox("Notice UP Count Overflow!!");
		strcpy( m_upNotice[count], (char*)(LPCTSTR)buff );
		count++;
	}
...
void CEbenezerDlg::SyncRegionTest(C3DMap* pMap, int rx, int rz, FILE* pfile, int nType)
{
	...
	for( ; Iter1 != Iter2; Iter1++ ) {
		...
		if(nType == 1)	{
			...
			if( !pUser ) {
				printf("SyncRegionTest : nid fail = %d\n", nid);
	...
	for(int i=0; i<MAX_USER; i++) {
		...
		if( pUser ) {
			...
			if(count == tot)	{
				...
				count = 0;
				send_tot++;
				//printf("AllNpcInfo - send_count=%d, count=%d\n", send_tot, count);
				::ZeroMemory(send_buff, sizeof(send_buff));
				//Sleep(320);
			}
		}
...
{
	if( m_bPointCheckFlag == FALSE)	return NULL;	// Æ÷ÀÎÅÍ ÂüÁ¶ÇÏ¸é ¾ÈµÊ
...
/*	if( m_byBattleOpen == NO_BATTLE )	{	// When Battlezone is closed, open it!
		if( nWeek == m_nBattleZoneOpenWeek && nTime == m_nBattleZoneOpenHourStart )	{	// ¼ö¿äÀÏ, 20½Ã¿¡ ÀüÀïÁ¸ open
			printf("ÀüÀï ÀÚµ¿ ½ÃÀÛ - week=%d, time=%d\n", nWeek, nTime);
			BattleZoneOpen(BATTLEZONE_OPEN);
//			KickOutZoneUsers(ZONE_FRONTIER);	// Kick out users in frontier zone.
		...
		if( nWeek == (m_nBattleZoneOpenWeek+1) && nTime == m_nBattleZoneOpenHourEnd )	{	// ¸ñ¿äÀÏ, 0½Ã¿¡ ÀüÀïÁ¸ close
			printf("ÀüÀï ÀÚµ¿ Á¾·á - week=%d, time=%d\n", nWeek, nTime);
			m_byBanishFlag = 1;
		}
	}	*/
	...
	if( m_byBanishFlag == 1 )	{		
		...
		if( m_sBanishDelay == 3 )	{
			...
			printf("ÀüÀï Á¾·á 1´Ü°è, m_bVictory=%d\n", m_bVictory);
		}
		else if( m_sBanishDelay == 8 )	{
			Announcement(DECLARE_BAN);
		}
		else if( m_sBanishDelay == 10 )	{
			printf("ÀüÀï Á¾·á 2´Ü°è - ¸ğµç À¯Àú ÀÚ±â ±¹°¡·Î °¡ \n");
			BanishLosers();
		}
		else if( m_sBanishDelay == 20 )	{
			printf("ÀüÀï Á¾·á 3´Ü°è - ÃÊ±âÈ­ ÇØÁÖ¼¼¿© \n");
...
void CEbenezerDlg::BattleZoneOpen( int nType )
{
	...
	if( nType == BATTLEZONE_OPEN ) {				// Open battlezone.
		...
		wsprintf(strLogFile, "Log/EventLog-%d-%d-%d.txt", time.GetYear(), time.GetMonth(), time.GetDay());
...
//
		if( !m_HomeArray.PutData(pHomeInfo->bNation, pHomeInfo) ) {
			printf("Home Info PutData Fail - %d\n", pHomeInfo->bNation );
			delete pHomeInfo;
			pHomeInfo = NULL;
		}
...
//	printf("Generate Item Serial : %I64d\n", serial.i);
	return serial.i;
}
...
BOOL CEbenezerDlg::LoadKnightsRankTable()
{
	...
	if( !KRankSet.Open() ) {
		printf("### KnightsRankTable Open Fail! ###\n");
		return TRUE;
	}
	if(KRankSet.IsBOF() || KRankSet.IsEOF()) {
		printf("### KnightsRankTable Empty! ###\n");
		return TRUE;
	}
	...
	while( !KRankSet.IsEOF() )	{
		...
		if( pKnights->m_byNation == KARUS )	{
			...
			if( pUser->m_pUserData->m_bKnights == nKnightsIndex	)	{
				...
				//memset( send_buff, NULL, 1024 );	send_index = 0;
				//printf("Elmo Captain - %s, rank=%d, index=%d\n", pUser->m_pUserData->m_id, nRank, nKnightsIndex);
			}
		}
	...
	printf("LoadKnightsRankTable Success\n");
...
void CEbenezerDlg::BattleZoneCurrentUsers()
{
	...
	if( !pMap )		return;
	if( m_nServerNo != pMap->m_nServerNo )	return;		// ÇöÀçÀÇ ¼­¹ö°¡ ¹èÆ²Á¸ ¼­¹ö°¡ ¾Æ´Ï¶ó¸é ¸®ÅÏ
	...
	//printf("---> BattleZoneCurrentUsers - karus=%d, elmorad=%d\n", m_sKarusCount, m_sElmoradCount);
...
int  CEbenezerDlg::GetKnightsAllBenefactorMembers(int knightsindex, char *temp_buff, int& buff_index, int type )
{
	if( knightsindex <= 0 ) return 0;			
	...
	if( type == 0 )	{
		for( i=0; i<MAX_USER; i++ ) {
			...
			if( pUser->m_pUserData->m_bKnights == knightsindex ) {		// °°Àº ¼Ò¼ÓÀÇ Å¬·£..
				SetShort( temp_buff, strlen(pUser->m_pUserData->m_id), buff_index );
				SetString( temp_buff, pUser->m_pUserData->m_id, strlen(pUser->m_pUserData->m_id), buff_index );
				SetDWORD( temp_buff, 0x00, buff_index);
				count++;
			}
		}	
	}
...
{
	for (int i = 0; i < (int)m_ZoneArray.size(); i++)
	{
		C3DMap* pMap = m_ZoneArray[i];
		if (pMap && pMap->m_nZoneNumber == zoneID)
			return pMap;
	}
	return NULL;
}

void CEbenezerDlg::Send_Region(char* pBuf, int len, C3DMap* pMap, int rx, int rz, CUser* pExcept)
{
	if (pMap == NULL)
		return;
	...
	for (int i = 0; i < MAX_USER; i++)
	{
		...
		if (pExcept && pUser == pExcept)
			continue;

		if (pUser->GetMap() == pMap)
		{
			pUser->Send(pBuf, len);
		}
	}
