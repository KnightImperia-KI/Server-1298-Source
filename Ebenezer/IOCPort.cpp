	...
	while(1)
	{
		...
		if(network_event.lNetworkEvents &FD_ACCEPT)
		{
			if(network_event.iErrorCode[FD_ACCEPT_BIT] == 0 ) 
			{
				...
				if(sid < 0) {
					printf("Accepting User Socket Fail - New Uid is -1\n");
					char logstr[1024]; memset( logstr, NULL, 1024 );
				...
				if( !pSocket ) {
					printf("Socket Array has Broken...\n");
					char logstr[1024]; memset( logstr, NULL, 1024 );
...
//					pIocport->PutOldSid( sid );				// Invalid sid must forbidden to use
				...
				// Þifreleme anahtarýný burada göndermek yerine, 
				// istemciye versiyon kontrol paketini gönderiyoruz.
				((CUser*)pSocket)->VersionCheck();
		...
		if(b || pOvl) 
		{
			if(b)
			{
				...
				case	OVL_RECEIVE:
					...
					if( !nbytes ) {
						printf("User Closed By 0 byte Notify...%d\n", WorkIndex);
						pSocket->CloseProcess();
				...
				if( pOvl ) {
					printf("User Closed By Abnormal Termination...%d\n", WorkIndex);
				}
				else {
					DWORD ioError = GetLastError();
					printf("User Closed By IOCP Error[%d] - %d \n", ioError, WorkIndex);
		...
		if(b || pOvl) 
		{
			if(b)
			{
				...
				case	OVL_RECEIVE:
					EnterCriticalSection( &g_critical );
					if( !nbytes ) {
						printf("AISocket Closed By 0 Byte Notify\n" );
						pSocket->CloseProcess();
						pIocport->RidIOCPSocket( pSocket->GetSocketID(), pSocket );
//						pIocport->PutOldSid( pSocket->GetSocketID() );		// Å¬¶óÀÌ¾ðÆ® ¼ÒÄÏÀº Sid °ü¸®ÇÏÁö ¾ÊÀ½
						LeaveCriticalSection( &g_critical );
						break;
					}
				...
				case	OVL_CLOSE:
					...
					printf("AISocket Closed By Close()\n" );
					pSocket->CloseProcess();
					pIocport->RidIOCPSocket( pSocket->GetSocketID(), pSocket );
//					pIocport->PutOldSid( pSocket->GetSocketID() );
				...
				if( pOvl ) {
					...
					printf("AISocket Closed By Abnormal Termination\n" );
					pSocket->CloseProcess();
					pIocport->RidIOCPSocket( pSocket->GetSocketID(), pSocket );
...
void CIOCPort::Init(int serversocksize, int clientsocksize, int workernum)
{
	...
	m_ClientSockArray = new CIOCPSocket2* [clientsocksize];		// ÇØ´ç ¼­¹ö°¡ Å¬¶óÀÌ¾ðÆ®·Î¼­ ´Ù¸¥ ÄÄÅÍ¿¡ ºÙ´Â ¼ÒÄÏ¼ö
...
BOOL CIOCPort::Listen(int port)
{
	...
	if (err == SOCKET_ERROR)
	{
		printf("FAIL : Set Socket RecvBuf of port(%d) as %d\n", port, socklen);
		return FALSE;
	}
	...
	if (err == SOCKET_ERROR)
	{
		printf("FAIL: Set Socket SendBuf of port(%d) as %d\n", port, socklen);
		return FALSE;
	}
	...
	if( m_hListenEvent == WSA_INVALID_EVENT ) {
		err = WSAGetLastError();
		printf("Listen Event Create Fail!! %d \n", err);
		return FALSE;
	}
	WSAEventSelect( m_ListenSocket, m_hListenEvent, FD_ACCEPT);

	printf("Port (%05d) initialzed\n", port);
...
{
	if (!hPort) {
		printf("ERROR : No Completion Port\n");
		return FALSE;
	}
...
void CIOCPort::PutOldSid(int sid)
{
	if( sid < 0 || sid > m_SocketArraySize ) {
		printf("recycle sid invalid value : %d\n", sid);
		return;
	}
...
CIOCPSocket2* CIOCPort::GetIOCPSocket(int index)
{
	...
	if( index > m_SocketArraySize ) {
		printf("InActiveSocket Array Overflow[%d]\n", index );
		return NULL;
	}
	if ( !m_SockArrayInActive[index] ) {
		printf("InActiveSocket Array Invalid[%d]\n", index );
		return NULL;
	}
	else
...
void CIOCPort::RidIOCPSocket(int index, CIOCPSocket2 *pSock)
{
	if( index < 0 || (pSock->GetSockType() == TYPE_ACCEPT && index >= m_SocketArraySize) || (pSock->GetSockType() == TYPE_CONNECT && index >= m_ClientSockSize) ) {
		printf("Invalid Sock index - RidIOCPSocket\n");
		return;
	}
