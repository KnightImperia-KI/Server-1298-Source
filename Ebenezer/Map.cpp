...
//	LogFileWrite("amp tile\r\n");
	LoadMapTile(hFile);
//	LogFileWrite("regene\r\n");
	LoadRegeneEvent(hFile);		// ÀÌ°Ç ³»°¡ Ãß°¡Çß½¿
...
void C3DMap::LoadObjectEvent(HANDLE hFile)
{
	...
	for( int i=0; i<iEventObjectCount; i++)
	{
		...
		ReadFile(hFile, &(pEvent->sBelong), 4, &dwNum, NULL);				// ¼Ò¼Ó : 0 -> ¹«¼Ò¼Ó
...
void C3DMap::LoadMapTile(HANDLE hFile)
{
	...
	m_ppnEvent = new short* [m_nMapSize];
	for (int i = 0; i < m_nMapSize; i++)
		m_ppnEvent[i] = new short[m_nMapSize];

	for (int x = 0; x < m_nMapSize; x++)
	{
		// 🔥 1298 FIX: short olarak oku (2 byte)
		ReadFile(hFile, m_ppnEvent[x], sizeof(short) * m_nMapSize, &dwNum, NULL);

		// güvenlik
		if (dwNum != sizeof(short) * m_nMapSize)
		{
			printf("MapTile Read ERROR! x=%d\n", x);
			return;
		}
	}

	printf("MapTile OK: MapSize=%d UnitDist=%f\n", m_nMapSize, m_fUnitDist);
}
void C3DMap::LoadRegeneEvent(HANDLE hFile)	
{
	...
	for( int i=0; i<iEventObjectCount; i++)
	{
		...
		ReadFile(hFile, &(pEvent->fRegenePosX), 4, &dwNum, NULL);	// Ä³¸¯ÅÍ ³ªÅ¸³ª´Â Áö¿ªÀÇ ¿Ş¾Æ·¡ÂÊ ±¸¼® ÁÂÇ¥ X
		ReadFile(hFile, &(pEvent->fRegenePosY), 4, &dwNum, NULL);	// Ä³¸¯ÅÍ ³ªÅ¸³ª´Â Áö¿ªÀÇ ¿Ş¾Æ·¡ÂÊ ±¸¼® ÁÂÇ¥ Y
		ReadFile(hFile, &(pEvent->fRegenePosZ), 4, &dwNum, NULL);	// Ä³¸¯ÅÍ ³ªÅ¸³ª´Â Áö¿ªÀÇ ¿Ş¾Æ·¡ÂÊ ±¸¼® ÁÂÇ¥ Z
		ReadFile(hFile, &(pEvent->fRegeneAreaZ), 4, &dwNum, NULL);	// Ä³¸¯ÅÍ ³ªÅ¸³ª´Â Áö¿ªÀÇ Z Ãà ±æÀÌ 
		ReadFile(hFile, &(pEvent->fRegeneAreaX), 4, &dwNum, NULL);	// Ä³¸¯ÅÍ ³ªÅ¸³ª´Â Áö¿ªÀÇ X Ãà ±æÀÌ

		if( pEvent->sRegenePoint < 0 ) continue;

		if( !m_ObjectRegeneArray.PutData(pEvent->sRegenePoint, pEvent) ) {
			printf("Regene Event PutData Fail - %d\n", pEvent->sRegenePoint);
			delete pEvent;
			pEvent = NULL;
		}

		// printf("Num: %d , (x, z, y) : (%f, %f, %f) , length_x : %f  length_z : %f\n", i,
...
void C3DMap::LoadWarpList(HANDLE hFile)
{
	...
	for(int i=0; i<WarpCount; i++) {
		...
		if( !m_WarpArray.PutData( pWarp->sWarpID, pWarp ) ) {
			printf("Warp list PutData Fail - %d\n", pWarp->sWarpID);
			delete pWarp;
			pWarp = NULL;
		}
...
void C3DMap::RegionUserAdd(int rx, int rz, int uid)
{
	...
	*pUid = uid;
	...
	m_ppRegion[rx][rz].m_RegionUserArray.m_UserTypeMap[uid] = pUid;
...
Iterator C3DMap::RegionUserRemove(int rx, int rz, int uid)
{
	...
	//printf("---- Region Remove(%d) : x=%d, z=%d, uid=%d ----\n", m_nZoneNumber, rx, rz, uid);
...
void C3DMap::RegionUserRemove(int rx, int rz, int uid)
{
	...
	auto& mp = m_ppRegion[rx][rz].m_RegionUserArray.m_UserTypeMap;
	mp.erase(uid);
...
{
	printf("RegionNpcAdd rx=%d rz=%d nid=%d\n",
		rx, rz, nid);
	...
	*pInt = nid;
	...
	pRegion->m_RegionNpcArray.PutData(nid, pInt);
