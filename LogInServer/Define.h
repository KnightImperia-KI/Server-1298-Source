...
#define MAX_WHAT_IS_NEWS_LENGTH		255
...
struct _NOTICE_INFO
{
	char strTitle[128];   // Baþlýk için 128 yeterli
	char strNotice[1024];  // Ýçerik için 1024 daha güvenli
	short sNoticeCount;
	_NOTICE_INFO() {
		memset(strTitle, 0x00, sizeof(strTitle));
		memset(strNotice, 0x00, sizeof(strNotice));
		sNoticeCount = 0;
	};
};
...
#define LS_WHATS_NEW				0xF6
#define LS_UNKNOWN					0xF7
