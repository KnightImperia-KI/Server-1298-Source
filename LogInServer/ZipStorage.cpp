...
void CZipStorage::Close(bool bAfterException)
{
	if (!bAfterException)
	{
		...
		if ((m_iSpanMode == tdSpan) && (m_bNewSpan))
		{
			...
			// if (m_file.m_hFile != (UINT)CFile::hFileNull)  <-- HATALI SATIR
			if (m_file.m_hFile != INVALID_HANDLE_VALUE)      // <-- DOÐRU SATIR
			{
				m_file.Close();
			}
		// ...
...
#ifdef _DEBUG // Dosya zaten kapalýysa assertion (hata bildirimi) almamak için
	// (UINT) yerine INVALID_HANDLE_VALUE veya (HANDLE) cast kullanýn
