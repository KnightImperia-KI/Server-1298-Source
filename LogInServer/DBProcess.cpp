...
int CDBProcess::IsPremiumUser(const char* id)
{
	SQLHSTMT		hstmt = NULL;
	SQLRETURN		retcode;
	TCHAR			szSQL[1024];
	memset(szSQL, 0x00, 1024);
	SQLSMALLINT		sDays = -1;
	SQLINTEGER		cbDays = SQL_NTS;

	wsprintf(szSQL, TEXT("{call ACCOUNT_PREMIUM(\'%s\',?)}"), id);

	retcode = SQLAllocHandle((SQLSMALLINT)SQL_HANDLE_STMT, m_VersionDB.m_hdbc, &hstmt);
	if (retcode == SQL_SUCCESS)
	{
		retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_OUTPUT, SQL_C_SSHORT, SQL_SMALLINT, 0, 0, &sDays, 0, &cbDays);
		if (retcode == SQL_SUCCESS)
		{
			retcode = SQLExecDirect(hstmt, (unsigned char*)szSQL, 1024);
			if (retcode != SQL_SUCCESS && retcode != SQL_SUCCESS_WITH_INFO) {
